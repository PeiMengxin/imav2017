...
{
    current_battery=*msg;
    //ROS_INFO("Battery: %f %f %f", current_battery.percentage, current_battery.current, current_battery.charge);    
}
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        if (current_battery.percentage<rtl_battery || (ros::Time::now()-fly_start_time)>ros::Duration(rtl_flytime))
        {
            rtl_enable = true;
            ROS_INFO("RTL Enable:");
            if (current_battery.percentage<rtl_battery)
            {
