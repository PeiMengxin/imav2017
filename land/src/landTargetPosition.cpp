...
int main(int argc, char *argv[])
{
    ...
    int ksize=1;
    Mat _tmpl = loadTemplate("/home/odroid/catkin_ws/src/land/config/templates.txt");
    cv::VideoWriter videowriter;
    videowriter.open("/home/odroid/workspace/video/2000.avi", CV_FOURCC('M', 'P', '4', '2'), 30, Size(640, 480));
        ...
        for(int i=0;i<2;++i)
        {
            ...
            if (contours.size()>0)
            {
                ...
                if(H_index>=0)
                {
                    ...
                    if (target_H.area()>400 
                        && (float)target_H.width/target_H.height>0.7
                        && (float)target_H.width/target_H.height<1.45)
                    {
